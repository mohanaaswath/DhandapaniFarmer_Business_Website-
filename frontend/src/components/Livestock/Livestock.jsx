...
import { Search, MapPin, Phone, Star, CheckBadge, Filter } from 'lucide-react';
import { livestock, getLivestockByCategory, searchLivestock } from '../../data/livestock';
...
const Livestock = () => {
  ...
  const filteredLivestock = useMemo(() => {
    let result =
      selectedCategory === 'all' ? livestock : getLivestockByCategory(selectedCategory);

    if (searchQuery) {
      result = searchLivestock(searchQuery);
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-lg text-dark-600">
            Find healthy, verified cattle, goats, buffalo, and more. All our livestock
            listings come from trusted sellers with complete health records.
          </p>
        </motion.div>
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {item.seller.verified && (
                      <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-primary-500/20">
                        <CheckBadge className="w-4 h-4 text-primary-400" />
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    ...
                    {item.milkYield !== 'N/A' && (
                      <div className="flex items-center gap-2 text-dark-600 col-span-2">
                        ...
                        <span className="text-sm text-gold-400">{item.milkYield}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-4 mb-4 p-3 rounded-xl bg-dark-200/50">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-white">{item.vaccination}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gold-500/10">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <span className="text-sm text-white">{item.seller.name}</span>
                        <div className="flex items-center gap-1">
                          <Star className="w-3 h-3 text-gold-400 fill-current" />
                          <span className="text-xs text-dark-600">{item.seller.rating}</span>
                        </div>
                      </div>
                    </div>
        ...
        {filteredLivestock.length === 0 && (
          ...
          >
            ...
            <h3 className="text-xl font-semibold text-white mb-2">No livestock found</h3>
